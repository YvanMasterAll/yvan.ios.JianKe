...
extension UIAlertController {
    ...
    ///   - vibrate: set true to vibrate the device while presenting the alert (default is false).
    ///   - completion: an optional completion handler to be called after presenting alert controller (default is nil).
    public func show(animated: Bool = true, vibrate: Bool = false, tapDismiss: Bool = false) {
        DispatchQueue.main.async {
            UIApplication.shared.keyWindow?.rootViewController?.present(self, animated: animated, completion: { [weak self] () in
                if tapDismiss {
                    let cover = self?.view.superview?.subviews[1]
                    let tap = UITapGestureRecognizer.init(target: self, action: #selector(self?.closeAlert))
