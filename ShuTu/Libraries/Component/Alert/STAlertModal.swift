...
public enum STAlertModalStyle {
    case none
    case fade
}

open class STAlertModal {
    ...
    fileprivate lazy var coverView: UIView = { //遮挡层
        ...
        //关闭事件
        let tapGes = UITapGestureRecognizer.init(target: STAlertModal.self, action: #selector(STAlertModal.hide))
        view.isUserInteractionEnabled = true
        view.addGestureRecognizer(tapGes)
        
        return view
    }()
    fileprivate var alertBox: STAlertBox! //模态框
    
    init(alertBox: STAlertBox) {
    ...
    fileprivate func setupUI() {
        if self.alertBox is STTestAlertBox {
            ...
            self.containerView.restorationIdentifier = STAlertModal.restorationIdentifier
        }
    }
    
    //MARK: - 显示 & 隐藏
    public func show(_ style: STAlertModalStyle) {
        //强引用
        let bself = self
        //隐藏
    ...
    @objc static fileprivate func hide() {
        if let window = UIApplication.shared.keyWindow {
            for view in window.subviews where view.restorationIdentifier == STAlertModal.restorationIdentifier {
